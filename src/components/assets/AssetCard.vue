<template>
	<div @click="viewAsset" class="relative flex cursor-pointer flex-col rounded-lg border border-gray-20 p-8">
		<div class="h-[10rem] w-full overflow-hidden rounded-lg bg-gray-200 group-hover:opacity-75 md:h-40">
			<img :src="asset.image_url" :alt="'product-15'"
				class="h-full w-full object-cover object-top lg:h-full lg:w-full" @error="onImageFail" />
		</div>
		...
		<div class="mt-5 flex w-full justify-between">
			...
			<div class="w-1/2 text-start" v-if="subscribed && subscription">
				<div>{{currentSalesValueGrowth}}%
					({{formatMoney(currentSalesValue)}})</div>
			...
			<div class="w-1/2 text-start" v-if="subscribed && subscription">
				<div>{{ percentageOwnership }}% ({{
				subscription?.units }}
					Unit{{subscription.units> 1? 's': ''}})</div>
		...
		<div class="absolute -top-3 left-0 flex justify-between">
			<div class="flex">
				...
				<div class="inline-flex">
					<div class="rounded-full border border-gray-70 bg-white py-1 px-4 text-xs text-gray-70">Own to Earn
					</div>
				</div>
			</div>
		</div>
...
const coOwnerCounter = computed(() => props.subscription?.project.funds.reduce((prev: any, curr: Subscription) => {
	if (prev[curr.userId]) {
