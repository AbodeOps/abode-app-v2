...
export default class TransactionService {
	...
	static async resolveBankAccount({ accountNumber, bankCode }: { accountNumber: string; bankCode: string }) {
		try {
			const res = await api.post('/bank-accounts/resolve', {
				account_number: accountNumber,
				bank_code: bankCode,
			});
			return res.data;
	...
	static async topup({
		...
		try {
			const res = await api.post('/wallets/topup', formData);
	...
	static async withdraw({ amount, bankId, otp }: { amount: string | number; bankId: string | number; otp: string }) {
		try {
